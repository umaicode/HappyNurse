...
class MedicationOrderControllerTest {
    ...
    @Test
    @DisplayName("GET /encounters/{encounterId}/orders - 조회 성공")
    void getOrders_성공() throws Exception {
        MedicationOrderItemResponse item = new MedicationOrderItemResponse(
                ...
                6L, "이조은",
                LocalDateTime.of(2026, 4, 27, 14, 0, 5),
                LocalDateTime.of(2026, 4, 27, 15, 30, 12));
        MedicationOrderListResponse response = new MedicationOrderListResponse(
                42L, 3L, "이승연", 1, List.of(item));

        given(medicationOrderService.getOrdersByEncounterId(42L)).willReturn(response);

        mockMvc.perform(get("/encounters/42/orders"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.encounterId").value(42))
                .andExpect(jsonPath("$.data.patientId").value(3))
                ...
                .andExpect(jsonPath("$.data.orders[0].createdAt").value("2026-04-27T14:00:05"))
                .andExpect(jsonPath("$.data.orders[0].updatedAt").value("2026-04-27T15:30:12"));
    }

    @Test
    @DisplayName("GET /encounters/{encounterId}/orders - 입원 없으면 404")
    void getOrders_실패_입원_없음() throws Exception {
        given(medicationOrderService.getOrdersByEncounterId(99L))
                .willThrow(new CustomException(ErrorCode.ENCOUNTER_NOT_FOUND));

        mockMvc.perform(get("/encounters/99/orders"))
