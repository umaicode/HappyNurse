...
public class WebappController {
    ...
    @Operation(
        ...
        description = "NFC 리다이렉트로 전달된 token으로 환자 정보를 조회합니다. 비로그인 접근 가능합니다."
    )
    @ApiResponses({
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "유효하지 않은 NFC 토큰"),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "활성 입원 정보 없음")
    })
    @GetMapping("/nfc/patients/entry")
    public ResponseEntity<ApiResponse<NfcEntryResponse>> nfcEntry(
        @Parameter(description = "NFC 리다이렉트 URL에 포함된 token", required = true)
        @RequestParam String token
    ) {
        return ResponseEntity.ok(ApiResponse.ok(webappService.getPatientEntry(token)));
    }
